...
const Search = () => {
  ...
  const getResults = async () => {
    if (previousValue !== "") {
      ...
      let jsonDataPages = await fetch(taurusData.root_url + "/wp-json/wp/v2/pages?search=" + previousValue).then(res => res.json()).then(res => {
        return res;
      });
      //Fetch posts based on search
      let jsonDataPosts = await fetch(taurusData.root_url + "/wp-json/wp/v2/posts?search=" + previousValue).then(res => res.json()).then(res => res);
