...
const MyNotes = () => {
  ...
  const deleteNote = async e => {
    ...
    //confrim
    if (confirm('Delete')) {
      axios__WEBPACK_IMPORTED_MODULE_0__["default"]["delete"](taurusData.root_url + `/wp-json/wp/v2/note/${ID}`, {
        headers
  ...
  const updateNote = async e => {
    ...
    try {
      await axios__WEBPACK_IMPORTED_MODULE_0__["default"].put(taurusData.root_url + `/wp-json/wp/v2/note/${ID}`, data, {
        headers
      }).then(deactivateEditFields);
    } catch (err) {
      console.log(err);
    }
  };
  ...
  const saveNewNote = async e => {
    ...
    try {
      await axios__WEBPACK_IMPORTED_MODULE_0__["default"].post(taurusData.root_url + `/wp-json/wp/v2/note/`, data, {
        headers
      }).then(res => console.log("Notes Created"));
    } catch (err) {
      console.log(err);
    }
  };
...
const Search = () => {
  ...
  const getResults = async () => {
    if (previousValue !== "") {
      ...
      let jsonData = await fetch(taurusData.root_url + "/wp-json/taurus/v1/search?term=" + previousValue).then(res => res.json()).then(res => {
        return res;
      });
        ...
        <div class="row">
          <div class="one-third">
            <h2 class="search-overlay__section-title">General Information</h2>
              ...
              ${jsonData.generalInfo.length > 0 ? jsonData.generalInfo.map(data => {
        return `
                    <li><a href='${data.link}'>${data.name}</a> ${data.postType == 'post' ? `by ${data.postAuthor}` : ''}</li>
                  `;
      }).join(' ') : "No General Info found"}
              </ul>
          </div>
          <div class="one-third">
            <h2 class="search-overlay__section-title">Programmes</h2>
            <ul class="link-list min-list">
              ${jsonData.programmes.length > 0 ? jsonData.programmes.map(data => {
        return `
                    <li><a href='${data.link}'>${data.name}</a></li>
                  `;
      }).join(' ') : `No Programmes found. <a href="${taurusData.root_url}/programmes">View All Programmes</a>`}
              </ul>
            <h2 class="search-overlay__section-title">Professors</h2>
            <ul class="professor-cards">
              ${jsonData.professors.length > 0 ? jsonData.professors.map(data => {
        return `
                  ...
                  `;
      }).join(' ') : `No Professors found. <a href="${taurusData.root_url}/professors">View All Professors</a>`}
              </ul>
          </div>
          <div class="one-third">
            <h2 class="search-overlay__section-title">Campuses</h2>
            <ul class="link-list min-list">
              ${jsonData.campuses.length > 0 ? jsonData.campuses.map(data => {
        return `
                    <li><a href='${data.link}'>${data.name}</a></li>
                  `;
      }).join(' ') : `No Campuses found. <a href="${taurusData.root_url}/campuses">View All Campuses</a>`}
              </ul>
            <h2 class="search-overlay__section-title">Events</h2>
              ${jsonData.events.length > 0 ? jsonData.events.map(data => {
        return `
                    ...
                    `;
      }).join(' ') : "No events found. <a href=''>View All Events</a>"}
          </div>
        </div>
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunktauras_theme"] = self["webpackChunktauras_theme"] || [];
